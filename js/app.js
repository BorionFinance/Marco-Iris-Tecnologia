...
const APP_VERSION=window.MARCO_APP_VERSION||'2.8.16';
...
function startRemoteRefresh(){
  ...
  REMOTE_REFRESH_TIMER=setInterval(()=>refreshFromDriveIfNewer({reason:'intervalo-otimizado'}),REMOTE_REFRESH_INTERVAL_MS);
...
async function validateLoginPin(){ return true; }
  ...
  $('#root').innerHTML=`<main class="login-screen ${entry==='left'?'screen-enter-left':''}">
    ...
    <section class="lock-shell">
      ...
      <form class="login-card login-card-compact" data-form="login" autocomplete="on">
        <div class="profile-option" aria-label="Perfil ${attr(p.name||'Marco')}"><div class="avatar">${esc((p.name||'M')[0])}</div><div class="spacer"><strong>${esc(p.name)}</strong><small>${esc(p.role||'Administrador')}</small></div><span class="profile-lock">${icon('cloud')}</span></div>
        ...
        <p class="login-security-note">Acesso somente pela conta Google autorizada. O Google Drive continua sendo a base oficial.</p>
      </form>
      <div class="lock-feature-row">
        ...
        <div class="lock-feature"><div class="lock-feature-icon">${icon('lock')}</div><div><strong>Segurança e confiabilidade</strong><small>Acesso protegido pela conta Google autorizada, sem senha adicional.</small></div></div>
...
function renderDashboard(){
  const d=data(),todayKey=today(),month=todayKey.slice(0,7),orders=d.serviceOrders.filter(o=>norm(o.registrationStatus||'Ativo').trim()!=='inativo'),open=orders.filter(o=>{const status=norm(o.status).replace(/[^a-z0-9]+/g,' ').trim();return !status.startsWith('conclu')&&!status.startsWith('finaliz')&&!status.startsWith('entreg')&&!status.startsWith('fechad')&&!status.startsWith('cancel')&&status!=='pronto para retirada'&&status!=='pronto para entrega';});
