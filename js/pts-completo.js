...
(() => {
  ...
  const SETTINGS_CATEGORIES = [
    ...
    ['backup','Backup e migração','AppSheet, Google Drive, pasta local e exportações.','cloud'],
  ...
  function migrateInitialStock(d){
    const migrate=(item,type)=>{
      ...
      if(initial&&!hasInitial){
        const id=MarcoIdentifiers.getNextEntityCode('MOV',d.stockMovements,'id',num(d.settings?.nextIds?.MOV));
        ...
        d.settings.nextIds.MOV=Math.max(num(d.settings.nextIds.MOV),parseSequence(id));
      }
  ...
  function syncHighWatermarks(d=data()){
    ...
    for(const [prefix,list] of Object.entries(groups)){
      ...
      d.settings.nextIds[prefix]=Math.max(num(d.settings.nextIds[prefix]),max);
    }
  ...
  normalizeState = function(){
    ...
    d.settings.modules={agenda:d.settings.modules?.agenda!==false,terms:d.settings.modules?.terms===true};
  ...
  nextCode = function(prefix,list,width=6,field='id'){
    ...
    const high=ENTITY_PREFIXES.has(normalized)?num(currentProfileSettings()?.nextIds?.[normalized]):0;
    if(ENTITY_PREFIXES.has(normalized))return MarcoIdentifiers.getNextEntityCode(normalized,list,field,high);
    let max=high;for(const item of list||[])max=Math.max(max,parseSequence(item?.[field]||item?.id||item?.code));
  ...
  function widgetLayout(id,index){
    ...
    const defaultSpan=screenBand()==='mobile'?12:screenBand()==='tablet'?6:6;
  ...
  renderDashboard = function(){
    ...
    const paidMonth=d.payments.filter(p=>normalizeText(p.type)==='receita'&&paymentIsPaid(p)&&monthKey(p.paymentDate)===month&&!isCancelledOrder(findOrder(p.orderId))).reduce((s,p)=>s+num(p.value),0);
    const expenses=d.payments.filter(p=>normalizeText(p.type)==='despesa'&&paymentIsPaid(p)&&monthKey(p.paymentDate)===month).reduce((s,p)=>s+num(p.value),0);
    ...
    const low=lowStockItems();const agendaOn=currentProfileSettings().modules.agenda;
    ...
    const kpis=[`<div class="card kpi"><div class="kpi-icon blue">${icon('orders')}</div><div><small>Ordens abertas</small><strong>${privacy(open.length)}</strong><div class="delta">${orders.length} OSVs ativas</div></div></div>`,`<div class="card kpi"><div class="kpi-icon green">${icon('finance')}</div><div><small>Receitas no mês</small><strong>${privacy(currency(paidMonth))}</strong><div class="delta">Saldo ${privacy(currency(paidMonth-expenses))}</div></div></div>`,agendaOn?`<div class="card kpi"><div class="kpi-icon orange">${icon('agenda')}</div><div><small>Agendas de hoje</small><strong>${privacy(appts.length)}</strong><div class="delta">${appts.filter(a=>normalizeText(a.status)==='confirmado').length} confirmados</div></div></div>`:'',`<div class="card kpi"><div class="kpi-icon red">${icon('stock')}</div><div><small>Estoque em alerta</small><strong>${privacy(low.length)}</strong><div class="delta">A receber ${privacy(currency(receivables.reduce((s,x)=>s+x.balance,0)))}</div></div></div>`].filter(Boolean).join('');
    const recentBody=recent.length?`<div class="list">${recent.map(o=>`<button class="list-row" data-action="view-order" data-id="${attr(o.id)}"><div class="list-row-main"><strong>${esc(o.id)} · ${esc(o.clientName||findClient(o.clientId)?.name||'Cliente')}</strong><small>${esc(o.equipmentType||'Equipamento não informado')} · ${formatDate(o.openedAt)}</small></div><div class="list-row-side">${statusBadge(o.status)}<small>${currency(o.total)}</small></div></button>`).join('')}</div>`:'<div class="empty">Nenhuma OSV cadastrada.</div>';
    const receiveBody=receivables.length?`<div class="list">${receivables.slice(0,12).map(x=>`<div class="list-row"><div class="list-row-main"><strong>${esc(x.client?.name||x.order.clientName||'Cliente')} · ${esc(x.order.id)}</strong><small>${x.dueDate?`Vencimento ${formatDate(x.dueDate)}`:'Sem vencimento combinado'} · saldo ${currency(x.balance)}</small></div>${statusBadge(x.status==='Parcial'&&x.overdue?'Parcial - vencido':x.status)}<div class="actions"><button data-action="new-payment" data-order="${attr(x.order.id)}" title="Adicionar pagamento">${icon('plus')}</button><button data-action="view-client" data-id="${attr(x.order.clientId)}" title="Abrir cliente">${icon('clients')}</button><button data-action="view-order" data-id="${attr(x.order.id)}" title="Abrir OSV">${icon('orders')}</button></div></div>`).join('')}</div>`:'<div class="empty">Nenhum cliente com saldo pendente.</div>';
    const stockBody=low.length?low.slice(0,12).map(x=>`<div class="list-row"><div class="list-row-main"><strong>${esc(x.name)}</strong><small>${x.type} · mínimo ${x.min}</small></div><div><span class="stock-health-badge ${x.health.tone}">${esc(x.health.label)}</span><small class="muted">Estoque ${x.stock}</small></div></div>`).join(''):'<div class="empty">Nenhum item em nível crítico ou de atenção.</div>';
    ...
    const widgets=[dashboardWidget('recent','Ordens recentes','Últimas movimentações da assistência.',recentBody,0),dashboardWidget('receivables','Clientes a Receber','Saldos em aberto, vencidos e parciais.',receiveBody,1),dashboardWidget('stock-alerts','Alertas de estoque','Produtos e insumos no mínimo.',stockBody,2)];
    if(agendaOn)widgets.splice(1,0,dashboardWidget('today-agenda','Agenda de hoje','Compromissos e visitas técnicas.',agendaBody,1));
    return `<section class="hero"><div class="hero-content"><div><h2>Olá, ${esc(activeProfile().name)}.</h2><p>OSVs, clientes a receber, agenda e estoque no mesmo fluxo operacional.</p></div><div class="hero-clock"><strong id="live-clock">--:--</strong><small id="live-date"></small></div></div></section><div class="dashboard-layout-toolbar">${DASHBOARD_LAYOUT_EDIT?`<button class="btn primary compact" data-action="save-dashboard-layout">Salvar</button><button class="btn secondary compact" data-action="cancel-dashboard-layout">Cancelar</button><button class="btn ghost compact" data-action="undo-dashboard-layout" ${DASHBOARD_LAYOUT_HISTORY.length?'':'disabled'}>Desfazer</button><button class="btn ghost compact" data-action="reset-dashboard-layout">Restaurar padrão</button><span class="muted">Layout ${screenBand()}: arraste ou use os controles.</span>`:`<button class="btn secondary compact" data-action="toggle-dashboard-layout">${icon('edit')} Editar módulos</button>`}</div><div class="grid kpis pts-kpis">${kpis}</div><div class="dashboard-widget-grid ${DASHBOARD_LAYOUT_EDIT?'layout-editing':''}" data-layout-container="dashboard">${widgets.join('')}</div>`;
  };

  function orderMatchesTemporal(order){
    const date=String(order.openedAt||'').slice(0,10);
    if(ORDER_FILTERS.mode==='Dia')return date===ORDER_FILTERS.day;
    if(ORDER_FILTERS.mode==='Mês')return date.slice(0,7)===ORDER_FILTERS.month;
    if(ORDER_FILTERS.mode==='Ano')return date.slice(0,4)===ORDER_FILTERS.year;
    return true;
  }
  function temporalControls(){
    const mode=ORDER_FILTERS.mode;
    return `<select id="order-temporal-mode" class="filter-control" aria-label="Filtro temporal">${['Nenhum','Dia','Mês','Ano'].map(x=>`<option ${x===mode?'selected':''}>${x}</option>`).join('')}</select>${mode==='Dia'?`<input id="order-filter-day" class="filter-control" type="date" value="${attr(ORDER_FILTERS.day)}">`:mode==='Mês'?`<input id="order-filter-month" class="filter-control" type="month" value="${attr(ORDER_FILTERS.month)}">`:mode==='Ano'?`<input id="order-filter-year" class="filter-control year-input" inputmode="numeric" maxlength="4" value="${attr(ORDER_FILTERS.year)}">`:''}`;
  }
  function quickOrderActions(order){
    return `<details class="quick-actions"><summary aria-label="Ações rápidas">${icon('menu',18)}</summary><div class="quick-actions-menu"><button data-action="new-payment" data-order="${attr(order.id)}">${icon('finance',16)} Adicionar pagamento</button><button data-action="view-current-pdf" data-id="${attr(order.id)}">${icon('pdf',16)} Visualizar PDF</button><button data-action="share-order" data-id="${attr(order.id)}">${icon('phone',16)} Enviar PDF</button><button data-action="view-client" data-id="${attr(order.clientId)}">${icon('clients',16)} Abrir cliente</button><button data-action="edit-order" data-id="${attr(order.id)}">${icon('edit',16)} Editar OSV</button></div></details>`;
  }
  ...
  function mitFinanceIndicators(period=''){
    const completed=data().serviceOrders.filter(orderNotCancelled).filter(o=>canonicalOperationalStatus(o.status)==='Concluída').filter(o=>!period||monthKey(o.completedAt||o.openedAt||o.createdAt)===period);
    ...
    const rows=data().payments.filter(p=>!paymentIsCancelled(p)).filter(p=>!period||monthKey(p.paymentDate||p.dueDate||p.createdAt)===period);
    ...
    const receivable=data().serviceOrders.filter(orderNotCancelled).filter(o=>!period||monthKey(orderFinancialInfo(o).dueDate||o.openedAt||o.createdAt)===period).reduce((sum,o)=>sum+orderFinancialInfo(o).balance,0);
  ...
  function sortedProductsForCatalog(source){const list=source.slice(),state=hydrateProductSort();if(!state.key||state.direction==='default')return list;const dir=state.direction;const indexed=list.map((item,index)=>({item,index}));indexed.sort((left,right)=>{const a=left.item,b=right.item;let cmp=0;if(state.key==='product')cmp=productTextCompare(a,b,'description',dir);else if(state.key==='supplier')cmp=productTextCompare(a,b,'supplier',dir);else if(state.key==='cost')cmp=numericWithMissing(a,b,x=>x.cost,dir);else if(state.key==='margin')cmp=numericWithMissing(a,b,x=>x.margin,dir);else if(state.key==='sale')cmp=numericWithMissing(a,b,x=>x.salePrice,dir);else if(state.key==='minimum')cmp=numericWithMissing(a,b,x=>x.minimumStock,dir);else if(state.key==='stock')cmp=stockHealthCompare(a,b,dir);return cmp||left.index-right.index;});return indexed.map(x=>x.item);}
  ...
  function movementsPts(){const mode=getViewMode('catalog'),list=[...data().stockMovements].filter(m=>matches(m.id,m.itemType,m.movementType,m.orderId,m.notes,itemForMovement(m)?.description)).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));return `<section class="card view-mode-content mode-${mode}"><div class="table-wrap"><table class="table"><thead><tr><th>ID</th><th>Data</th><th>Item</th><th>Tipo</th><th>Quantidade</th><th>OSV</th><th>Observação</th><th></th></tr></thead><tbody>${list.map(m=>`<tr><td><strong>${esc(m.id)}</strong></td><td>${formatDate(m.date)}</td><td><strong>${esc(itemForMovement(m)?.description||m.productId||m.supplyId||'—')}</strong><small class="muted">${esc(m.itemType)} · ${m.sourceItemId?'Automática':'Manual'}</small></td><td>${statusBadge(m.movementType)}</td><td>${num(m.quantity)}</td><td>${m.orderId?`<button class="code-link" data-action="view-order" data-id="${attr(m.orderId)}">${esc(m.orderId)}</button>`:'—'}</td><td>${esc(m.notes||'—')}</td><td><div class="actions">${m.sourceItemId?`<button data-action="view-order" data-id="${attr(m.orderId)}">${icon('link')}</button>`:`<button data-action="edit-stock-movement" data-id="${attr(m.id)}">${icon('edit')}</button><button data-action="delete-stock-movement" data-id="${attr(m.id)}">${icon('trash')}</button>`}</div></td></tr>`).join('')||'<tr><td colspan="8"><div class="empty">Nenhuma movimentação cadastrada.</div></td></tr>'}</tbody></table></div></section>`;}
  ...
  function openCostUpdate(kind,id){const item=kind==='product'?data().products.find(x=>x.id===id):data().supplies.find(x=>x.id===id);if(!item)return;openModal('Atualizar custo',`<form data-form="cost-update" data-kind="${attr(kind)}" data-id="${attr(id)}" data-margin="${attr(num(item.margin))}" data-price-touched="false"><div class="form-grid one-column"><div class="field"><label>Cadastro</label><input readonly value="${attr(item.description)}"></div><div class="field money-field"><label>Custo atual</label><input readonly value="${attr(currency(item.cost))}"></div>${field('Novo custo','newCost',num(item.cost).toFixed(2),'number','step="0.01" min="0" required')}${kind==='product'?`<div class="field"><label>Margem atual</label><input readonly value="${attr((num(item.margin)*100).toFixed(1)+'%')}"></div><div class="field"><label>Novo preço de venda</label><input name="newPrice" type="number" step="0.01" min="0" value="${attr(marginPrice(num(item.cost),num(item.margin)).toFixed(2))}"></div>`:''}</div><div class="form-actions"><button type="button" class="btn secondary" data-action="close-modal">Cancelar</button><button class="btn primary">Atualizar custo</button></div></form>`);};
  async function saveCostUpdate(form){const kind=form.dataset.kind,id=form.dataset.id,item=kind==='product'?data().products.find(x=>x.id===id):data().supplies.find(x=>x.id===id),v=Object.fromEntries(new FormData(form));if(!item)throw new Error('Cadastro não encontrado.');const oldCost=num(item.cost),newCost=num(v.newCost);data().costHistory.push({id:`CST-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,catalogType:kind==='product'?'Produto':'Insumo',catalogId:id,date:nowIso(),oldCost,newCost});item.cost=newCost;item.costUpdatedAt=today();if(kind==='product'){const preserved=marginPrice(newCost,num(item.margin));item.salePrice=form.dataset.priceTouched==='true'?num(v.newPrice):preserved;item.margin=item.salePrice>0?(item.salePrice-newCost)/item.salePrice:0;item.priceUpdatedAt=today();}await persist('Custo atualizado',`${id}: ${currency(oldCost)} → ${currency(newCost)}`);closeModal();renderView();toast('Custo e histórico atualizados.');}
  ...
  async function viewCurrentOrderPdf(orderId){
    ...
    if(latest.sourceFingerprint!==orderPdfFingerprint219(orderId))throw new Error('PDF desatualizado — gere novamente.');
  ...
  function settingsPreferencesCard(s){return `<section class="card"><div class="card-header"><div><h2>Preferências visuais e operacionais</h2><p>Persistidas por perfil e separadas dos registros do sistema.</p></div></div><label class="list-row"><div class="list-row-main"><strong>Ocultar valores no painel</strong><small>Mostra •••• no lugar de valores.</small></div><input type="checkbox" data-setting="dashboardPrivacy" ${s.dashboardPrivacy?'checked':''}></label><label class="list-row"><div class="list-row-main"><strong>Impedir estoque negativo</strong><small>Exige confirmação ou bloqueia saída acima do saldo.</small></div><input type="checkbox" data-setting="preventNegativeStock" ${s.preventNegativeStock?'checked':''}></label><label class="list-row"><div class="list-row-main"><strong>Salvar automaticamente no Google Drive</strong></div><input type="checkbox" data-setting="autosaveGoogle" ${s.autosaveGoogle?'checked':''}></label><label class="list-row"><div class="list-row-main"><strong>Salvar automaticamente na pasta local</strong></div><input type="checkbox" data-setting="autosaveFolder" ${s.autosaveFolder?'checked':''}></label><button class="btn ghost" data-action="reset-all-layouts">Restaurar todos os layouts padrão</button></section>`;}
  ...
  function settingsContent(active,{c,drive,diag,s,lastMigration,activeMigration}){
    ...
    if(active==='backup')return `<div class="settings-category-content settings-grid"><section class="card migration-card"><div class="card-header"><div><h2>Migração do AppSheet</h2><p>Pré-análise, simulação, vínculos, idempotência e rollback.</p></div>${lastMigration?statusBadge(lastMigration.rolledBack?'Desfeita':'Última importação concluída'):statusBadge('Pronta')}</div><div class="migration-actions"><button class="btn primary" data-action="open-migration-picker">${icon('upload')} Selecionar arquivos</button>${activeMigration?`<button class="btn danger" data-action="rollback-migration">Desfazer última migração ativa</button>`:''}<button class="btn secondary" data-action="export-migration-log">Exportar log técnico</button></div><div class="migration-summary"><span>${data().migrationHistory.length} execução(ões)</span><span>${s.migrationKeys.length} chave(s) idempotentes</span><span>Backup automático antes da gravação</span></div></section><section class="card full-settings-card"><div class="card-header"><div><h2>Google Drive, pasta local e backups</h2><p>Dados, fotos, PDFs e anexos em pastas separadas.</p></div>${drive?statusBadge('Google conectado'):statusBadge('Google desconectado')}</div><div class="toolbar"><div class="toolbar-left">${drive?`<button class="btn primary" data-action="sync-google">${icon('cloud')} Sincronizar</button><button class="btn secondary" data-action="load-google">Carregar Drive</button><button class="btn danger" data-action="disconnect-google">Desconectar</button>`:`<button class="btn primary" data-action="connect-google">${icon('cloud')} Conectar Google</button>`}<button class="btn secondary" data-action="manual-save">${icon('save')} Salvar tudo</button><button class="btn secondary" data-action="diagnose-drive">${icon('check')} Testar instalação</button><button class="btn secondary" data-action="export-json">${icon('download')} Exportar JSON</button><button class="btn secondary" data-action="local-backups">Backups locais</button><button class="btn secondary" data-action="connect-folder">${icon('folder')} Conectar pasta local</button></div></div><div class="migration-summary"><span>Autosave local: ${s.autosaveFolder?'ativo':'inativo'}</span><span>Autosave Drive: ${s.autosaveGoogle?'ativo':'inativo'}</span><span>Backups Drive: autosave-1…20 e forcesave-1…20</span><span>Integração: Borion_Integracoes/marco-iris.bridge.json</span></div></section></div>`;
  ...
  handleAction = async function(btn){
    ...
    try{
      ...
      if(a==='clear-order-filters'){ORDER_FILTERS={status:'Todos',mode:'Nenhum',day:today(),month:today().slice(0,7),year:String(new Date().getFullYear())};renderView();return;}
      if(a==='clear-finance-filter'){FINANCE_FILTER.month='';renderView();return;}
      ...
      if(a==='widget-width'){pushDashboardHistory();const store=dashboardLayoutStore(),id=btn.dataset.id,l=store[id]||widgetLayout(id,0),spans=screenBand()==='mobile'?[12]:[4,6,8,12],i=Math.max(0,spans.indexOf(l.span));l.span=spans[Math.max(0,Math.min(spans.length-1,i+num(btn.dataset.dir)))];store[id]=l;renderView();return;}
  ...
  document.addEventListener('input',e=>{
    const t=e.target;if(t.closest?.('form[data-form="cost-update"]')){const form=t.closest('form'),price=form.elements.newPrice;if(t.name==='newPrice')form.dataset.priceTouched='true';if(t.name==='newCost'&&price&&form.dataset.priceTouched!=='true'){window.MarcoMoney?.setValue(price,marginPrice(num(t.value),num(form.dataset.margin)));}}if(t.dataset.mask==='document')t.value=maskDocument(t.value);if(t.dataset.mask==='phone')t.value=maskPhone(t.value);if(t.dataset.mask==='zip')t.value=maskZip(t.value);
  ...
  document.addEventListener('change',async e=>{
    ...
    if(t.id==='documents-filter-date'){DOCUMENT_FILTER.date=t.value;renderView();return;}
    if(t.id==='order-status-filter'){ORDER_FILTERS.status=t.value;renderView();return;}if(t.id==='order-temporal-mode'){ORDER_FILTERS.mode=t.value;renderView();return;}if(t.id==='order-filter-day'){ORDER_FILTERS.day=t.value;renderView();return;}if(t.id==='order-filter-month'){ORDER_FILTERS.month=t.value;renderView();return;}if(t.id==='order-filter-year'){ORDER_FILTERS.year=t.value;renderView();return;}if(t.id==='finance-filter-month'){FINANCE_FILTER.month=t.value;renderView();return;}
  ...
  renderLogin = function(entry=''){
    baseRenderLogin(entry);
    const screen=$('.login-screen');
    if(!screen||screen.querySelector('.dev-test-entry'))return;
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='dev-test-entry';
    btn.title='Entra direto no sistema, sem Google e sem PIN — acesso de testes do proprietário.';
    btn.setAttribute('aria-label','Entrar sem conta');
    btn.textContent='Entrar sem conta';
    btn.addEventListener('click',()=>{completeLogin('dev','');});
  ...
  function latestOfficialPdfMeta219(orderId){
    const order=findOrder(orderId);return (order?.pdfs||[]).filter(m=>m.official!==false&&!m.legacy).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')))[0]||null;
  }
  ...
  async function setupOrderForm219(form,draft=null){
    ...
    try{
      ...
      const latest=orderId?latestOfficialPdfMeta219(orderId):null,current=latest&&latest.sourceFingerprint===orderPdfFingerprint219(orderId);
      form.dataset.hadPdf=latest?'1':'0';
      setPdfState219(form,current?'ready':latest?'dirty':'idle',current?'PDF pronto':latest?'PDF desatualizado — gere novamente.':'Gere o PDF quando a OSV estiver pronta.');
  ...
  async function getLatestOfficialPdf219(orderId){
    ...
    return {order,meta,blob,file,current:meta.sourceFingerprint===orderPdfFingerprint219(orderId)};
  }
  ...
  async function shareOrderPdfWithMessage219({orderId,pdfFile,message}){
    ...
    if(canNative){
      try{
        ...
        addAudit('Compartilhamento iniciado',`${order.id} · confirmação depende do usuário`);await MarcoStorage.save(STATE);
    ...
    addAudit('WhatsApp aberto para envio',`${order.id} · PDF baixado para anexação manual`);await MarcoStorage.save(STATE);
  ...
  generatePdfForOrder=async function(orderId,share=false){
    ...
    const task=(async()=>{
      ...
      try{
        ...
        newMeta={id:`pdf_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,orderId:order.id,kind:'pdf',official:true,legacy:false,fileName,localKey:record.id,driveFileId:'',webViewLink:'',createdAt:nowIso(),sourceFingerprint:orderPdfFingerprint219(order.id),schemaVersion:3,templateId:result.templateId||order.pdfTemplateId||currentProfileSettings().defaultPdfTemplateId||'',templateVersion:result.templateVersion||1,pageCount:result.pageCount||0};
        if(GoogleDriveMarco.isConfigured()){try{const remote=await GoogleDriveMarco.uploadBlob(pdfFile,'pdfs',fileName);newMeta.driveFileId=remote.id;newMeta.webViewLink=remote.webViewLink||'';}catch(e){console.warn('PDF salvo localmente; Drive pendente:',e);toast('PDF salvo localmente; envio ao Drive ficou pendente.','warn');}}
        const oldOfficial=(order.pdfs||[]).filter(m=>m.official!==false&&!m.legacy);order.pdfs=(order.pdfs||[]).filter(m=>m.official===false||m.legacy);order.pdfs.push(newMeta);
        await persist('PDF oficial gerado',`${order.id} · ${fileName}`);
        for(const old of oldOfficial){if(old.localKey)await MarcoStorage.deleteMedia(old.localKey).catch(e=>console.warn(e));if(old.driveFileId&&GoogleDriveMarco.isConfigured())await GoogleDriveMarco.trash(old.driveFileId).catch(e=>console.warn(e));}
      ...
      }catch(e){console.error('Falha ao gerar PDF oficial:',{orderId,error:e});order.pdfs=previousPdfs;if(record?.id)await MarcoStorage.deleteMedia(record.id).catch(()=>{});setPdfState219(form,'error','Não foi possível gerar o PDF.');setSaveStatus('Falha ao gerar PDF','warn');toast('Não foi possível gerar o PDF.','error');throw e;}
