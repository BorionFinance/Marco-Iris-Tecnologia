...
(() => {
  ...
  function readPaymentRow(row){
    ...
    const fee=/d[eé]bito|cr[eé]dito|outro/i.test(paymentMethod)?num(g('fee')?.value):0;
    return {id:row.dataset.paymentId||'',value:num(g('value')?.value),paymentMethod,paymentDate:g('paymentDate')?.value||'',dueDate:g('planned')?.checked?(g('dueDate')?.value||''):'',fee,notes:g('notes')?.value||''};
  }
  ...
  savePaymentForm = async function(form){
    ...
    const priorCancellation=old?.cancelledAt?{cancelledAt:old.cancelledAt,cancelReason:old.cancelReason||'',restoredAt:nowIso()}:null;const cancellationHistory=Array.isArray(old?.cancellationHistory)?old.cancellationHistory.slice():[];if(priorCancellation&&!v.cancelled&&!cancellationHistory.some(x=>x.cancelledAt===priorCancellation.cancelledAt))cancellationHistory.push(priorCancellation);const item={id,code:old?.code||id,orderId:v.orderId||'',clientId:order?.clientId||old?.clientId||'',type,paymentMethod:v.paymentMethod,value,fee:/débito|crédito|outro/i.test(v.paymentMethod)?num(v.fee):0,grossValue:value+(/débito|crédito|outro/i.test(v.paymentMethod)?num(v.fee):0),paymentDate,dueDate,planned:!!dueDate&&!paymentDate,expenseName:type==='Despesa'?String(v.expenseName||'').trim():'',localPurchase:type==='Despesa'?String(v.localPurchase||'').trim():'',expenseCategory:type==='Despesa'?(String(v.expenseCategory||'Outro').trim()||'Outro'):'',notes:v.notes,externalReference:old?.externalReference||`${v.orderId||'AVULSO'}:${id}`,createdAt:old?.createdAt||nowIso(),updatedAt:nowIso(),cancelledAt:v.cancelled?old?.cancelledAt||nowIso():'',cancelReason:v.cancelled?(old?.cancelReason||'Cancelado pelo usuário'):'',cancellationHistory,status:'Em aberto'};item.status=v.cancelled?'Cancelado':recordFinancialStatus(item);if(old)Object.assign(old,item);else{clearPendingPaymentDeletion(activeProfile().id,id);data().payments.push(item);}await persist(old?'Lançamento atualizado':'Lançamento criado',`${item.id} · ${currency(item.value)}`);closeModal();renderView();toast('Lançamento salvo e vínculos recalculados.');
  };
