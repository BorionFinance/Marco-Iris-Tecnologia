...
/* Marco Iris v2.5.4 — revisão pós-migração solicitada por Marco
...
(() => {
  ...
  const SAVE={requested:0,confirmed:0,running:false,retryTimer:0,retryDelay:5000,lastError:null};
  ...
  function repairProfile(pd){
    ...
    if(!pd.settings.migrations.marcoReviewV254?.completedAt){
      ...
      if(has292&&!has291){
        ...
        for(const media of removedMedia)if(media?.driveFileId)pd.settings.pendingDriveCleanup.push({id:`cleanup_v254_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,orderId:'OSV-000292',mediaId:media.id||'',driveFileId:media.driveFileId,fileName:media.fileName||'',createdAt:new Date().toISOString(),reason:'OSV de teste removida pela revisão v2.5.4'});
      }
      ...
      for(const payment of pd.payments||[])if(payment.orderId==='OSV-000002'){
        payment.value=0;payment.grossValue=0;payment.fee=0;payment.status='Cancelado';payment.active=false;payment.cancelledAt=payment.cancelledAt||new Date().toISOString();payment.cancelReason='OSV-000002 marcada como cancelada na revisão v2.5.4';
      }
      ...
      pd.settings.migrations.marcoReviewV254={completedAt:new Date().toISOString(),version:VERSION,notes:'OSV de teste removida condicionalmente; OSV-000002 e OSV-000057 reparadas; PDFs históricos marcados.'};
  ...
  function savePending(){return SAVE.confirmed<SAVE.requested||SAVE.running||!!SAVE.retryTimer;}
  function setPendingFlag(){CLOUD_ONLY_COMMITTING=savePending();CLOUD_PENDING_LOCAL=savePending();}
  function scheduleFullRetry(){
    clearTimeout(SAVE.retryTimer);SAVE.retryTimer=setTimeout(()=>{SAVE.retryTimer=0;setPendingFlag();runSaveQueue().catch(error=>console.error('[V254_SAVE_RETRY]',error));},SAVE.retryDelay);
    setPendingFlag();
  }
  async function runSaveQueue(){
    if(SAVE.running)return;SAVE.running=true;setPendingFlag();
    try{
      while(SAVE.confirmed<SAVE.requested){
        const target=SAVE.requested;
        try{
          const result=await flushCloudState('fila-v254',{backup:false,retryMedia:true});
          SAVE.confirmed=target;SAVE.lastError=null;LAST_CONFIRMED_STATE=clone(STATE);await MarcoStorage.saveSyncBase?.(STATE);
          ...
          if(error?.baseCommitted){
            SAVE.confirmed=target;LAST_CONFIRMED_STATE=clone(STATE);await MarcoStorage.saveSyncBase?.(STATE);setSaveStatus('Dados no Drive · integração com Borion pendente','warn');scheduleCloudRetry('fila-v254');continue;
          }
  ...
  persist=async function(action='',detail='',opts={}){
    ...
    SAVE.requested++;setSaveStatus('Salvando em segundo plano…','warn');setPendingFlag();queueMicrotask(()=>runSaveQueue().catch(error=>console.error('[V254_BACKGROUND_SAVE]',error)));
    return {queued:true,revision:SAVE.requested,cloud:true,drive:'pending',errors:[]};
  };
  ...
  generatePdfForOrder=function(orderId,share=false){
    ...
    const task=Promise.resolve().then(()=>generatePdfBase(orderId,false)).catch(error=>{console.error('[V254_PDF]',error);toast(error?.message||'Não foi possível gerar o PDF.','error');}).finally(()=>PDF_TASKS.delete(orderId));
  ...
  function hideRedundantDetailHeader(){requestAnimationFrame(()=>{const modal=document.querySelector('#modal-root .modal');if(modal?.querySelector('.detail-hero'))modal.classList.add('detail-modal-v254');});}
  ...
  function enhanceTableSorting(root=document){
    ...
    schema.labels.forEach((label,index)=>{
      ...
      th.innerHTML=`<button type="button" class="table-sort-button-v254 ${active?'is-active':''}" data-action="table-sort-v254" data-section="${attr(section)}" data-column="${index}" title="${attr(!active?`Ordenar ${label}: maior para menor`:state.direction==='desc'?`${label}: maior para menor. Clique para menor para maior.`:`${label}: menor para maior. Clique para voltar ao padrão.`)}"><span>${esc(label)}</span><span aria-hidden="true">${indicator}</span></button>`;
    });
  ...
  window.MarcoV254={version:VERSION,reconcileNextIds,repairProfile,enhanceTableSorting,saveStatus:()=>({...SAVE}),pdfTasks:PDF_TASKS};
  window.MarcoAppBoot?.();
})();
